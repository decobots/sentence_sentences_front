...
var app = (function () {
    ...
    function create_if_block$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(div, file$2, 73, 8, 1503);
    		},
    		m: function mount(target, anchor) {
    ...
    function create_if_block_1$1(ctx) {
    	...
    	const punctuation = new Punctuation({
    			props: { symbol: /*letter*/ ctx[7].value },
    			$$inline: true
    		});
    	...
    	const block = {
    		...
    		p: function update(ctx, dirty) {
    			...
    			if (dirty & /*state*/ 1) punctuation_changes.symbol = /*letter*/ ctx[7].value;
    ...
    // (117:12) {#if letter.type=='letter'}
    function create_if_block$2(ctx) {
    	let current;

    	const input = new Input({
    			props: {
    				answer: /*letter*/ ctx[7].value,
    				correct: /*letter*/ ctx[7].correct,
    				focus: /*letter*/ ctx[7].focus,
    				position: /*i*/ ctx[9]
    			},
    			$$inline: true
    		});

    	input.$on("guess", /*onGuess*/ ctx[3]);
    	input.$on("error", /*error_handler*/ ctx[6]);
    	...
    	const block = {
    		...
    		p: function update(ctx, dirty) {
    			const input_changes = {};
    			if (dirty & /*state*/ 1) input_changes.answer = /*letter*/ ctx[7].value;
    			if (dirty & /*state*/ 1) input_changes.correct = /*letter*/ ctx[7].correct;
    			if (dirty & /*state*/ 1) input_changes.focus = /*letter*/ ctx[7].focus;
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "if",
    		source: "(117:12) {#if letter.type=='letter'}",
    		ctx
    	});
    ...
    function create_each_block(ctx) {
    	...
    	function select_block_type(ctx, dirty) {
    		if (/*letter*/ ctx[7].type == "letter") return 0;
    		if (/*letter*/ ctx[7].type == "punctuation") return 1;
    		if (/*letter*/ ctx[7].type == "whitespace") return 2;
    		return -1;
    	}
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(116:4) {#each state as letter,i }",
    		ctx
    	});
    ...
    function create_fragment$4(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(p, file$4, 111, 0, 3233);
    			add_location(br, file$4, 112, 0, 3262);
    			attr_dev(div, "class", "text svelte-s20lbl");
    			add_location(div, file$4, 113, 0, 3269);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    		m: function mount(target, anchor) {
    			...
    			current = true;
    			dispose = listen_dev(p, "click", /*help*/ ctx[4], false, false, false);
    		},
    		p: function update(ctx, [dirty]) {
    			const hang_changes = {};
    			if (dirty & /*errors*/ 2) hang_changes.errors = /*errors*/ ctx[1];
    			if (dirty & /*win*/ 4) hang_changes.win = /*win*/ ctx[2];
    			hang.$set(hang_changes);

    			if (dirty & /*state, onGuess, errors*/ 11) {
    ...
    function instance$3($$self, $$props, $$invalidate) {
    	...
    	onMount(async () => {
    		...
    		$$invalidate(5, line = await res.json());
    	});
    		...
    		if (correctGuess) {
    			for (let i = 0; i < state.length - 1; i++) {
    				if (state[i].type === "letter" && state[i].value.toLowerCase() === event.detail.text.toLowerCase()) {
    					$$invalidate(0, state[i].correct = true, state);

    					if (i >= pos && first) {
    						// set flag to focus next input
    						$$invalidate(0, state[i].focus = "next", state);
    			...
    			if (!first) {
    				for (let j = pos; j < state.length - 1; j++) {
    					if (state[j].focus === "next") {
    						$$invalidate(0, state[j].focus = false, state);

    						for (let k = j + 1; k < state.length - 1; k++) {
    							if (state[k].type === "letter" && state[k].correct === false) {
    								$$invalidate(0, state[k].focus = "autofocus", state);
    								break;
    							}
    						}
    			...
    			$$invalidate(1, errors = errors - 1);
    		}

    		($$invalidate(0, state), $$invalidate(5, line));
    		$$invalidate(2, win = !Boolean(state.find(obj => obj.correct === false)));
    	}
    	...
    	return [state, errors, win, onGuess, help, line, error_handler];
    }
